...
import { getPublicFeedback } from "../../api/userService";

import { Spinner } from "flowbite-react";

export default function RatingsPublic() {
  ...
  }, [axiosPrivate, userId, currentPage, filter, reloadToken]);
  ...
  const maskEmail = (email) => {
    ...
    const visibleStart = localPart.substring(0, Math.min(2, localPart.length));
    const visibleEnd = domain.substring(0, Math.min(2, localPart.length));
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-2xl font-bold text-gray-900">
                      {maskUserName(userInfo.fullName)}
                    </h1>
                    <p className="text-gray-600">{maskEmail(userInfo.email)}</p>
                  </div>
                </div>
              </div>
            )}
